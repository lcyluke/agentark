...
- **Parent page must be shared with the integration** before creating child pages. If POST /v1/pages returns 404 with a valid page_id, the parent hasn't been manually connected in Notion UI (`...` → Connect to). Child pages and databases inherit access from the parent once connected.
- **Multi-file batch sync:** When uploading several docs as sub-pages under a common parent, each file gets its own POST /v1/pages with the same parent.page_id. Read the file content into shell, json-escape it (e.g. with `python3 -c "import json,sys; print(json.dumps(sys.stdin.read()))"`), then embed in the markdown field of the POST body.
