...
apex fleet init-fleet -n <name> -r <repo> # ⚓ 初始化多Mac舰队 (Origin)
apex fleet join-fleet -r <repo>          # 🖥 加入舰队 (Worker)
apex fleet nodes                         # 🖥 查看舰队节点状态
apex fleet sync                          # 🔄 Git拉取配置 (Worker)
apex fleet sync --push                   # 🔄 Git推送配置 (Origin)
...
- `references/multi-mac-fleet-architecture.md` — Multi-Mac fleet: Origin/Worker, Git sync, node heartbeat, Apex fleet commands
...
- `references/authorization-delegation-v3.md` — v3 delegation system, dual-approval flow, audit-guardian, scope boundaries, all 18 REST endpoints
- `references/autonomous-daemon-deployment.md` — AutonomousEngine daemon, dual scheduling architecture, task registration
- `references/multi-mac-fleet-architecture.md` — Multi-Mac fleet design: Origin/Worker, Git sync, dual-approval Origin transfer
...
38. **Multi-Mac fleet: Apex fleet commands for Origin/Worker management.** Use `apex fleet init-fleet/join-fleet/nodes/sync`. Origin runs cron+dashboard, Workers pull config via git. GitHub repo: `lcyluke/hermes-fleet-config`. See `references/fleet-multi-mac-implementation.md` for code locations and setup guide.
...
| ⚓ Fleet | `hermes-fleet-config` | Origin (Mac-A) | 3-5 Nodes | 多Mac舰队管理 |
