...
from agentark.orchestration.crew import crew as crew_group
...
class AliasedGroup(click.Group):
    ...
    def get_command(self, ctx, cmd_name):
        ...
        from agentark.interface.aliases import resolve_alias
...
def setup(quick: bool, check_mode: bool, model: Optional[str],
    ...
    """
    from agentark.interface.setup_wizard import run_setup
...
def init(name: str, dir: str, quick: bool, project_type: str):
    ...
    """
    if quick:
        from agentark.interface.project_factory import quick_init
        quick_init(name, Path(dir).resolve(), console)
    else:
        from agentark.interface.project_factory import ProjectFactory
...
def dashboard(host: str, port: int):
    """📊 Start Web Dashboard"""
    try:
        from agentark.interface.web import run_dashboard
...
def demo(port: int, host: str, no_browser: bool, skip_tasks: bool, overwrite: bool):
    ...
    try:
        from agentark.cli.commands.demo import run_demo
...
def model_detect():
    ...
    try:
        from agentark.cli.commands.model_detect import detect_models, auto_configure
        from rich.table import Table
        from rich.panel import Panel
...
def team_template(template_name: str, setup_script: bool):
    ...
    from agentark.interface.hermes_sync import create_team_from_template, get_team_setup_script, TEAM_TEMPLATES
...
def team_sync(profile_name: str, hermes_name: str, display: str):
    ...
    from agentark.interface.hermes_sync import sync_profile_to_hermes
...
def team_sync_all():
    ...
    from agentark.interface.hermes_sync import sync_all_profiles
...
def fleet_monitors():
    ...
    from agentark.core.project_template import list_all_monitors, get_all_monitor_count
...
def monitor_status(json_output: bool, watch: int):
    ...
    from agentark.interface.monitor import cmd_status
...
def monitor_skills():
    ...
    from agentark.interface.monitor import cmd_skills
    cmd_skills()
...
def pm_dashboard(project: str):
    ...
    from agentark.interface.pm_engine import PMEngine
    from agentark.interface.pm_render import render_dashboard
    engine = PMEngine()
...
def pm_schedule(project: str):
    ...
    from agentark.interface.pm_engine import PMEngine
    from agentark.interface.pm_render import render_schedule
    engine = PMEngine()
...
def pm_health():
    ...
    from agentark.interface.pm_engine import PMEngine
    from agentark.interface.pm_render import render_health
    engine = PMEngine()
...
def pm_timeline(project: str):
    ...
    from agentark.interface.pm_engine import PMEngine
    from agentark.interface.pm_render import render_timeline
    engine = PMEngine()
...
def pm_profile(agent_name: str):
    ...
    from agentark.interface.pm_engine import PMEngine
    from agentark.interface.pm_render import render_profile
    engine = PMEngine()
...
def project_create(project_key: str, name: str, type: str, size: str,
    ...
    from agentark.core.project_template import (
        build_smart_template, summarize_template,
        ProjectType, ProjectSize,
    )
    from agentark.interface.hermes_sync import sync_profile_to_hermes, ROLE_SOULS
    ...
    if not click.confirm(f"\n🚀 创建 {summary['total_agents']} 个Agent?", default=True):
        ...
        return
    from agentark.core.project_template import create_project
...
def project_list():
    ...
    from agentark.core.project_template import LEGACY_TEMPLATES, summarize_template
...
def integrate_hermes_profiles():
    ...
    from agentark.interface.hermes_sync import list_hermes_profiles
...
def integrate_monitor_check(file: str, url: str, pattern: str):
    ...
    try:
        from agentark.orchestration.monitor import Monitor, WatcherRule
        m = Monitor()
        if file:
...
def config_show():
    ...
    from agentark.core.config import get_config
    import json
    cfg = get_config()
...
def config_set(key: str, value: str):
    ...
    from agentark.core.config import get_config, save_config
    parts = key.split(".", 1)
    if len(parts) != 2:
...
def config_path_cmd():
    ...
    from agentark.core.config import config_path
...
def doctor_cmd(fix: bool, json_output: bool):
    ...
    from agentark.interface.doctor import Doctor, render_diagnostic, render_json
    d = Doctor()
    if fix:
        results = d.auto_fix()
        for r in results:
...
def tutorial_cmd():
    ...
    from agentark.interface.tutorial import run_tutorial
    run_tutorial()
...
def version_cmd():
    ...
    from agentark.interface.logo import render_mini
    render_mini(console, "0.5.1")
    from agentark.interface.version import cmd_version
    cmd_version()
...
def update_cmd():
    ...
    from agentark.interface.version import cmd_update
    cmd_update()
...
def theme_list():
    ...
    from agentark.interface.skin_engine import SkinEngine
    from rich.table import Table
    engine = SkinEngine()
...
def theme_set(name: str):
    """Set active theme"""
    from agentark.interface.skin_engine import get_engine
    engine = get_engine()
...
def alias_list():
    """List all aliases"""
    from agentark.interface.aliases import list_aliases
    from rich.table import Table
...
def alias_add(name: str, command: str):
    """Add a custom alias"""
    from agentark.interface.aliases import add_alias
...
def alias_remove(name: str):
    ...
    from agentark.interface.aliases import remove_alias
