...
def skill_match_cmd(task: str, difficulty: str = "L2", required_skills: str = ""):
    ...
    from apex.cli.commands.skill_mgmt import match_cmd as _m
    _m(task, difficulty, required_skills)


def skill_evaluate_cmd():
    """Run skill evaluation pipeline."""
    from apex.cli.commands.skill_mgmt import evaluate_cmd as _e
    _e()


def skill_sync_cmd(agent_name: str):
    """Generate SKILL.md for agent's Hermes profile."""
    from apex.cli.commands.skill_mgmt import sync_cmd as _s
    _s(agent_name)
...
def economy_status_cmd():
    """View economy status."""
    from apex.cli.commands.economy import status_cmd as _es
    _es()


def economy_classify_cmd(task: str):
    """Test task classification and routing."""
    from apex.cli.commands.economy import classify_cmd as _ec
    _ec(task)
...
def evolution_status_cmd():
    ...
    from apex.cli.commands.evolution import status_cmd as _es
    _es()


def evolution_agent_cmd(name: str):
    """Agent evolution report."""
    from apex.cli.commands.evolution import agent_cmd as _ea
    _ea(name)
...
def autonomous_status_cmd():
    ...
    from apex.cli.commands.autonomous import status_cmd as _as
    _as()


def autonomous_start_cmd():
    """Start the autonomous engine (7x24 mode)."""
    from apex.cli.commands.autonomous import start_cmd as _as
    _as()


def autonomous_stop_cmd():
    """Stop the autonomous engine."""
    from apex.cli.commands.autonomous import stop_cmd as _as
    _as()


def autonomous_pause_cmd():
    """Pause task dispatch (heartbeat continues)."""
    from apex.cli.commands.autonomous import pause_cmd as _ap
    _ap()


def autonomous_resume_cmd():
    """Resume task dispatch."""
    from apex.cli.commands.autonomous import resume_cmd as _ar
    _ar()


def autonomous_schedule_cmd(name: str, cron: str, task: str, agent: str = ""):
    """Schedule a recurring task."""
    from apex.cli.commands.autonomous import schedule_cmd as _asc
    _asc(name, cron, task, agent)


def autonomous_unschedule_cmd(task_id: str):
    """Remove a scheduled task."""
    from apex.cli.commands.autonomous import unschedule_cmd as _auc
    _auc(task_id)


def autonomous_alerts_cmd():
    """Show unresolved alerts."""
    from apex.cli.commands.autonomous import alerts_cmd as _aac
    _aac()
