...
def chain_cmd(goal: str, pipeline_type: str = "dev"):
    ...
    from apex.orchestration import Chain
    pm = ProfileManager()
    try:
...
def debate_cmd(topic: str, agents: int = 3):
    ...
    try:
        ...
        for i in range(min(agents, 3)):
            ...
            pm.save(profile)
            from apex.orchestration.debate import DebatePosition
...
def supervise_cmd(goal: str, workers: int = 3):
    ...
    try:
        pm = ProfileManager()
        from apex.orchestration import Supervisor
...
def pipeline_normal_cmd(requirement: str, project: str = "finopsai", auto_confirm: bool = True):
    ...
    from apex.cli.commands.pipeline_cmds import pipeline_normal_cmd as _pn
...
def pipeline_direct_cmd(task: str, agent: str, project: str = "finopsai", priority: int = 1):
    ...
    from apex.cli.commands.pipeline_cmds import pipeline_direct_cmd as _pd
    _pd(task, agent, project, priority)


def pipeline_status_cmd(pipeline_id: str):
    """📊 View pipeline status."""
    from apex.cli.commands.pipeline_cmds import pipeline_status_cmd as _ps
    _ps(pipeline_id)


def pipeline_confirm_cmd(pipeline_id: str):
    """✅ Confirm pipeline to continue."""
    from apex.cli.commands.pipeline_cmds import pipeline_confirm_cmd as _pc
    _pc(pipeline_id)
